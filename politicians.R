...
liberal = c("npr", "politico","huff","buzzfeed", "cnn", "msnbc", "vox", "npr", "nytimes")
newspapers = c("nytimes", "wsj", "latimes", "usatoday", "washingtonpost", "bostonglobe")
conservative = c("foxnews", "dailycaller", "infowars", "breitbart")
...
# make this a function also, get avg sentiment of headlines /stories 
  ...
  num_source_articles$politician = factor(num_source_articles$politician, levels = num_source_articles$politician[order(num_source_articles$num_articles, decreasing=TRUE)])
  num_source_articles = num_source_articles[order(num_source_articles$num_articles, decreasing=TRUE),]
  p = ggplot(num_source_articles, aes(x=politician, y=num_articles, fill=politician)) + geom_bar(stat="identity") 
...
PlotOverTime <- function(dt, name, weekly_article_stats, max_date) {
  name.ts = xts(dt$num_articles,dt$date)
  articles = apply.weekly(name.ts,sum)
  articles = articles[1:length(articles)-1] # remove last entry for incomplete week
  #dev.on()
  png(sprintf('./%s_time.png', name))
  print(plot(articles))
...
top_contenders = rbind(warren,harris,sanders, beto, biden)
top_contenders$politician = factor(top_contenders$politician)

data_name = "top_contenders"
p = ggplot(top_contenders, aes(x=date, y=num_articles, color=politician)) + geom_point() 
p = p + geom_line()
...
bottom_contenders = rbind(yang, booker, gillibrand, klobuchar, buttigieg)
...
p = p + geom_line()
p = p + ggtitle("Candidates New Twitter Followers per Month (millions)")
p = p + theme(plot.title = element_text(hjust = 0.5))
p = p + theme(axis.text.x=element_text(angle=90, hjust=1,face="bold"))
p = p + theme(axis.text=element_text(size=16,face="bold"), axis.title=element_text(size=13), axis.title.x=element_blank())
p = p + theme(plot.title = element_text(size=18,face="bold"))
